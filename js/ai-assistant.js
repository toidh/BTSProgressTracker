...
Không đoán bừa khi thiếu sở cứ — nêu rõ cần thêm thông tin gì.`,
  ...
  async fetchKnowledgeBase() {
    try {
      // Force it to turn yellow to indicate it started!
      const nm = document.getElementById("ai-name-text");
      if (nm) nm.style.color = "yellow";
      ...
      if (!result.success) {
        alert("Lỗi từ server: " + JSON.stringify(result).substring(0,200));
        return;
      }

      if (result.errors && result.errors.length > 0) {
        alert("Cảnh báo khi đọc tài liệu: " + result.errors[0]);
      }
      ...
      console.error(e);
      alert("Lỗi mạng hoặc script: " + e.message);
    }
  },
